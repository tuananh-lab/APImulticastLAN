...
int main() {
    ...
    // Bind UDP socket to receive unicast messages from server
    ...
    if (bind(udp_socket, (struct sockaddr *)&udp_client_addr, sizeof(udp_client_addr)) == -1) {
        perror("UDP bind failed");
        exit(EXIT_FAILURE);
    }

    printf("Waiting for unicast message from server...\n");

    // Receive unicast message from server
    ssize_t bytes_received;
    socklen_t udp_server_addr_len = sizeof(udp_server_addr);
    bytes_received = recvfrom(udp_socket, buffer, MAX_BUF_SIZE, 0, (struct sockaddr *)&udp_server_addr, &udp_server_addr_len);
    if (bytes_received == -1) {
        perror("Receive failed");
        exit(EXIT_FAILURE);
    }
    ...
    buffer[bytes_received] = '\0';
    printf("Received unicast message from server: %s\n", buffer);
